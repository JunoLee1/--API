...
export function RankingsPage() {
  ...
  useEffect(() => {
    ...
    analysisApi.getRankings({
      ...
      .then(setRankings)
      .catch(() => {
        toast.error(t('rankingsPage.loadFailed'))
        setRankings([])
      })
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="border-b px-6 py-4 shrink-0">
        <h1 className="text-lg font-semibold tracking-tight">{t('rankingsPage.title')}</h1>
        <p className="text-sm text-muted-foreground mt-0.5">{t('rankingsPage.subtitle')}</p>
      </div>
      ...
      <div className="border-b px-6 py-3 flex items-center gap-3 shrink-0 bg-muted/30">
        ...
        <Select value={compType} onValueChange={(v) => setCompType(v)}>
          ...
          <SelectContent>
            {RANKABLE_TYPES.map((ct) => (
              <SelectItem key={ct} value={ct}>{t(`competitionType.${ct}`)}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="flex-1 overflow-auto">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-48 gap-2 text-muted-foreground">
            <p className="text-sm">{t('rankingsPage.noData')}</p>
          </div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead>{t('rankingsPage.team')}</TableHead>
                <TableHead className="w-10 text-center">{t('rankingsPage.played')}</TableHead>
                <TableHead className="w-10 text-center">{t('rankingsPage.wins')}</TableHead>
                <TableHead className="w-10 text-center">{t('rankingsPage.draws')}</TableHead>
                <TableHead className="w-10 text-center">{t('rankingsPage.losses')}</TableHead>
                <TableHead className="w-12 text-center">{t('rankingsPage.goalsFor')}</TableHead>
                <TableHead className="w-12 text-center">{t('rankingsPage.goalsAgainst')}</TableHead>
                <TableHead className="w-12 text-center">{t('rankingsPage.goalDiff')}</TableHead>
                <TableHead className="w-14 text-center font-semibold">{t('rankingsPage.points')}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {rankings.map((row) => {
