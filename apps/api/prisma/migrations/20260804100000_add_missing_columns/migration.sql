...
-- Ensure LeagueLevel type exists (may not if earlier migrations never ran)
DO $$ BEGIN
  CREATE TYPE "LeagueLevel" AS ENUM ('K3', 'K_LEAGUE_2', 'K_LEAGUE_1', 'EPL', 'OTHER');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- Handle League table: drop old structure (nation_id) and create new (level/year)
DO $$
BEGIN
...
-- ClubLeague (only create if Club table exists)
DO $$
BEGIN
