...
router.get("/stats", auth, (req, res, next) => {
  const { role, foRole } = req.user!;
  if (!canReadHR(role, foRole)) return next(new AppError(403, "FORBIDDEN"));
  next();
}, controller.getStats);
...
router.get("/player/:playerId", auth, (req, res, next) => {
  const { role } = req.user!;
  if (role !== 'GUARDIAN' && !canReadHR(role, req.user!.foRole)) {
...
router.post("/issue", auth, (req, res, next) => {
  const { role, foRole } = req.user!;
  if (!canWriteHR(role, foRole)) return next(new AppError(403, "FORBIDDEN"));
  next();
}, controller.issueMonthlyFees);

router.patch("/:id/submit-proof", auth, (req, res, next) => {
  const { role } = req.user!;
  if (role !== 'GUARDIAN' && !canWriteHR(role, req.user!.foRole)) {
    return next(new AppError(403, "FORBIDDEN"));
  }
  next();
