...
## Test Plan
- [ ] PLAYER 로그인 → 사이드바에 '전술 분석' 메뉴 표시 확인
- [ ] PLAYER 접속 → 목록에 POST_MATCH + CONFIRMED 항목만 표시 확인
- [ ] PLAYER 접속 → 자신이 라인업에 없는 경기 분석이 목록에 미노출 확인
- [ ] PLAYER 접속 → '새 분석' 버튼 미노출 확인
- [ ] PLAYER 접속 → 행 클릭 시 수정 다이얼로그 미열림 확인
- [ ] PLAYER가 다른 선수의 경기 분석 URL 직접 접근 시 404 반환 확인
- [ ] COACHING_STAFF 로그인 → 기존 전체 목록 정상 표시 확인
- [ ] ADMIN 로그인 → 생성/수정/확정 기능 정상 동작 확인"
```
