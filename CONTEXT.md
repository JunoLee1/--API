...
**운영/행정팀 (EQUIPMENT_MANAGER·FINANCE_MANAGER 등):**
일반 운영비 집행, 식대·간식 관리, 구장 및 차량 대여 실무, 회계 및 수납 관리. ERP 내 선택적 역할 — 미배정 시 GM이 대리 수행.
...
**성인팀 감독 및 코치진 (HEAD_COACH + ATTACKING/DEFENSIVE/GK/ASSISTANT_COACH 등):**
1군 전술 수립, 훈련 세션 운영, 포지션별(공격·수비·GK) 파트별 지도 및 경기 후 리뷰 분석. `coachingRole`로 세분화.

**유소년(Youth) 감독 및 코칭스태프:**
유소년 선수 육성, '포지션 한계 폐지' 철학 적용(단일 포지션 비율 ≥ 80% 선수 모니터링), 성장 리포트(GrowthReport) 관리. `User.teamId → Youth Team`.

**의무팀 (Medical Staff):**
선수 부상 등록, 가중치 기반 의무 보고 및 재활·복귀(RTP) 프로세스 관리. `Injury`, `MedicalExpense` 쓰기 권한 보유.
...
분기별 재무 수치를 GM/ADMIN이 CSV 임포트 또는 수동 입력으로 저장하고, ERP 내부 집계와 결합하여 구단 운영 안정성 지수를 산출하는 단위.

**구단 운영 안정성 지수 (Club Operational Stability Index):**
재무 안정성, 운영 효율성, 수익 다양성, 팬 기반 지표 4개 축을 종합 평가하는 복합 지수. 각 축은 온디맨드 KPI 계산으로 도출되며 저장하지 않는다.
- 재무 안정성: 유동비율, 부채비율, 이자보상배율
- 운영 효율성: 인건비비율, 영업이익률, 순이익률
- 수익 다양성: 모기업 의존도, 티켓 수입(관중회전율 × 평균 객단가) 비중
- 팬 기반 지표: 관중회전율, 시즌권 보유 수 추이
...
| 부서명 | 주요 업무 |
|--------|----------|
| 시설관리팀 – 그라운드 매니지먼트 | 훈련·경기 전 잔디 상태(패인 곳, 배수) 사전 체크 및 시설공단/지자체 보수 요청 |
| 시설관리팀 – 테크니컬 서포트 | 구단 소유 훈련 장비/대여 기기 점검, 구장 시설(전광판·음향·냉난방) 이상 시 관리소 조율 |
| 시설관리팀 – 환경미화 및 안전관리 | 대관 조건 내 청소 포함 여부 사전 확인, 경기 후 락커룸·관람석 청소 규정 준수 |
| 인사팀 – 인력 및 계약 관리 | 코칭스태프·프런트 직원 채용, 근로계약 체결 및 갱신 스케줄 관리 |
...
**필드:**
- `name: String` — 직원명
- `departmentId: Int? → Department` — 소속 부서 FK (Department 모델 참조, onDelete: SetNull)
